...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(account -> account.getBirthday().getMonth() == birthdayMonth).toList();
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getEmail().substring(account.getEmail().lastIndexOf("@") + 1)));
    }
    ...
    public List<Account> sortByFirstAndLastNames() {
        return accounts.stream().sorted(Comparator.comparing(account -> account.getFirstName() + account.getLastName())).toList();
    }
    ...
    public BigDecimal getBalanceByEmail(String email) {
        BigDecimal bigDecimal = accounts.stream().filter(account -> account.getEmail().equals(email)).map(Account::getBalance).findFirst().orElse(null);
        if (bigDecimal == null) {
    ...
    public Map<Month, BigDecimal> groupTotalBalanceByCreationMonth() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getCreationDate().getMonth(), Collectors.mapping(Account::getBalance, Collectors.reducing(BigDecimal.ZERO, BigDecimal::add))));
    }
