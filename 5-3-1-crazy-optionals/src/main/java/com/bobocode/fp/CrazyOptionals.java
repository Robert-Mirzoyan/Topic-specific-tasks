...
public class CrazyOptionals {
    ...
    public static void deposit(AccountProvider accountProvider, BigDecimal amount) {
        if (accountProvider.getAccount().isPresent() && amount.compareTo(BigDecimal.ZERO) > 0) {
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream().mapToDouble(account -> account.getCreditBalance().orElse(BigDecimal.ZERO).doubleValue()).sum();
    }
}
